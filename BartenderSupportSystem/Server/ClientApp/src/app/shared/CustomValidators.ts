...
export class CustomValidators {
    ...
    static validateArrayMinLength(min: number) {
        return (control: AbstractControl): { [key: string]: any } | null => {
            if (control && control instanceof FormArray && (<FormArray>control).length >= min) {
                return null;
            }
            else {
    ...
    static validateArrayMaxLength(max: number) {
        return (control: AbstractControl): { [key: string]: any } | null => {
            if (control && control instanceof FormArray && (<FormArray>control).length <= max) {
                return null;
            }
            else {
    ...
    static atLeastOneCorrectAnswerExists() {
        return (control: AbstractControl): { [key: string]: any } | null => {
            if (control && control instanceof FormArray) {
                let checkList: boolean[] = new Array();
