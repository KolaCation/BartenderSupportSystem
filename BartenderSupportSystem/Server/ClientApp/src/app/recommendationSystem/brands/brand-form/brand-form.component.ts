...
export class BrandFormComponent implements OnInit {
  ...
  fillFormWithValuesToEdit(id: number): void {
    this._brandService.getBrand(id).subscribe(
      (brand: IBrand) => {
        const countryName: string = CountryConverter.fromCodeToName(
  ...
  mapFormValuesToModel(): void {
    ...
    const countryCode: string = CountryConverter.fromNameToCode(
      this.brandForm.get('countryOfOrigin').value
    );
